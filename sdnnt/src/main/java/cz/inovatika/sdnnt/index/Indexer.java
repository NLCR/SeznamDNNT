...
public class Indexer {
    ...
    static List<String> identifierFields = Arrays.asList("/identifier", "/datestamp", "/setSpec", "/controlFields/001",
            "/controlFields/003", "/controlFields/005", "/controlFields/008");
    ...
    public static JSONObject add(String collection, List<SolrInputDocument> recs, boolean merge, boolean update,
            String user) {
        return add(collection, recs, merge, update, user, getClient());
    }

    public static JSONObject add(String collection, List<SolrInputDocument> recs, boolean merge, boolean update,
            String user, SKCUpdateSupportServiceImpl updatesupportService) {
        return add(collection, recs, merge, update, user, getClient(), updatesupportService);
    }

    public static JSONObject add(String collection, List<SolrInputDocument> recs, boolean merge, boolean update,
            String user, SolrClient client) {
        return add(collection, recs, merge, update, user, client, null);
    }

    public static JSONObject add(String collection, List<SolrInputDocument> recs, boolean merge, boolean update,
            String user, SolrClient client, SKCUpdateSupportServiceImpl updatesupportService) {
    ...
    static SolrInputDocument mergeWithHistory(String jsTarget, SolrDocument docCat, SolrInputDocument historyDoc,
        ...
        try {
            ...
            if (new JSONArray(fwPatch.toString()).length() > 0) {
                ...
                JSONObject changes = new JSONObject().put("forward_patch", new JSONArray(fwPatch.toString()))
    ...
    public static SolrDocumentList findById(String identifier, SolrClient client) {
        // JSONObject ret = new JSONObject();
        try {
            SolrQuery query = new SolrQuery("*").addFilterQuery("identifier:\"" + identifier + "\"").setRows(1)
                    .setFields("*");
    ...
    public static SolrDocumentList find(String source, SolrClient client) {
        ...
        try {
            ...
            String q = "(controlfield_001:\"" + sdoc.getFieldValue("controlfield_001") + "\"" + " AND marc_040a:\""
                    + sdoc.getFieldValue("marc_040a") + "\"" + " AND controlfield_008:\""
                    + sdoc.getFieldValue("controlfield_008") + "\")" + " OR marc_020a:\""
                    + sdoc.getFieldValue("marc_020a") + "\"" + " OR marc_015a:\"" + sdoc.getFieldValue("marc_015a")
                    + "\"" + " OR dedup_fields:\"" + sdoc.getFieldValue("dedup_fields") + "\"";

            SolrQuery query = new SolrQuery(q).setRows(20).setFields("*,score");
            return client.query("catalog", query).getResults();
    ...
    public static JSONObject approveInImport(String identifier, String newRaw, String user) {
        ...
        try {
            ...
            SolrQuery q = new SolrQuery("*").setRows(1).addFilterQuery("id:\"" + impNew.id + "\"");
    ...
    public static Map<User, List<Map<String, String>>> checkNotifications(UserController controler, String interval) {
        ...
        try {
            ...
            for (User u : usersByNotificationInterval) {
                LOGGER.log(Level.INFO, String.format("Sending notifications: %s for user  %s", interval,
                        u.getJmeno() + " " + u.getPrijmeni() + "(" + u.getEmail()));
        ...
        try {
            ...
            SolrQuery q = new SolrQuery("*").setRows(1000).setSort("identifier", SolrQuery.ORDER.desc)
                    .addFilterQuery(fqCatalog).addFilterQuery(fqJoin).setFields("identifier,datum_stavu,nazev,dntstav");
            boolean done = false;
            while (!done) {
                ...
                for (SolrDocument doc : docs) {
                    ...
                    SolrQuery qUser = new SolrQuery("*").setRows(1)
                            .addFilterQuery("{!join fromIndex=notifications from=user to=username} identifier:\""
                                    + doc.getFirstValue("identifier") + "\"");
                    QueryResponse uResponse = getClient().query("users", qUser);
                    List<User> users = uResponse.getResults().stream().map(User::fromSolrDocument)
    ...
    public static void changeStavDirectBatchVersion(List<MarcRecord> marcRecords, SolrClient solrClient, String newStav,
            String licence, String poznamka, String user) throws IOException, SolrServerException {
        try {
            ...
            for (MarcRecord mr : marcRecords) {
                ...
                new HistoryImpl(solrClient).log(mr.identifier, before.toString(), mr.toJSON().toString(), user,
                        DataCollections.catalog.name(), null, false);
            }
...
//      });
    ...
    public JSONObject save(String id, JSONObject newRaw, String user) {
        ...
        try (SolrClient solr = new HttpSolrClient.Builder(opts.getString("solr.host", "http://localhost:8983/solr/"))
                .build()) {
            SolrQuery q = new SolrQuery("*").setRows(1).addFilterQuery("identifier:\"" + id + "\"").setFields("raw");
    ...
    public JSONObject compare(String id) {
        ...
        try (SolrClient solr = new HttpSolrClient.Builder(opts.getString("solr.host", "http://localhost:8983/solr/"))
                .build()) {
            SolrQuery q = new SolrQuery("*").setRows(1).addFilterQuery("identifier:" + id).setFields("marc_035a,raw");
            ...
            SolrQuery q2 = new SolrQuery("*").setRows(1).addFilterQuery("controlfield_001:" + sysno).setFields("raw");
    ...
    public JSONObject mergeId(String sourceCore, String id, String user) {
        ...
        try (SolrClient solr = new HttpSolrClient.Builder(opts.getString("solr.host", "http://localhost:8983/solr/"))
            ...
            SolrQuery q2 = new SolrQuery("*").setRows(1).addFilterQuery("controlfield_001:" + sysno)
            ...
            if (docsCat.getNumFound() == 0) {
                ...
                ret.append("errors", "Found more than one record in catalog: " + docsCat.stream()
                        .map(d -> (String) d.getFirstValue("identifier")).collect(Collectors.joining()));
                return ret;
            } else {
    ...
    public JSONObject mergeCore(String sourceCore, String user, String from) {
        ...
        try (SolrClient solr = new HttpSolrClient.Builder(opts.getString("solr.host", "http://localhost:8983/solr/"))
            ...
            int indexed = 0;
            String cursorMark = CursorMarkParams.CURSOR_MARK_START;
            SolrQuery q = new SolrQuery("*").setRows(1000).setSort("identifier", SolrQuery.ORDER.asc)
                    .setFields("identifier,raw");
            if (from != null) {
    ...
    private static SolrInputDocument mergeRaw2(String jsDnt, String identifier, String sysno, String sigla,
        ...
        try {
            ...
            SolrQuery q2 = new SolrQuery("*").setRows(1)
                    ...
                    .addFilterQuery("controlfield_008:\"" + controlfield_008 + "\"").setFields("identifier,raw");
            SolrDocumentList docsCat = getClient().query("catalog", q2).getResults();
            if (docsCat.getNumFound() == 0) {
                // ret.append("errors", "Record " + identifier + " with sysno: " + sysno + " not
                // found in catalog");
                LOGGER.log(Level.WARNING, "Record {0} with sysno: {1} not found in catalog",
                        new Object[] { identifier, sysno });
                return null;
            } else if (docsCat.getNumFound() > 1) {
                LOGGER.log(Level.WARNING, "Found more than one record in catalog: {0} -> {1}",
                        new Object[] { identifier, docsCat.stream().map(d -> (String) d.getFirstValue("identifier"))
                                .collect(Collectors.joining()) });
                // ret.append("errors", "Found more than one record in catalog: " + identifier +
                // " with sysno: " + sysno);
                return null;
            }
            SolrDocument docCat = docsCat.get(0);
...
//      ret.put("backward_patch", new JSONArray(bwPatch.toString()));
    ...
    public static JSONObject followRecord(String identifier, String user, NotificationInterval interval,
            boolean follow) {
        JSONObject ret = new JSONObject();
        try {
            if (follow) {
