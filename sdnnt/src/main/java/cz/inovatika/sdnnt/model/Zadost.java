...
public class Zadost implements NotNullAwareObject {
    ...
    public List<String> inAnyStates(String transitionName) {
       return  itemsInDifferentStates(transitionName, "approved", "rejected");
    }

    public List<String> notRejected(String transitionName) {
        return  itemsInDifferentStates(transitionName,  "rejected");
    }

    private boolean allItemsInOneState(String transitionName, String itemState) {
        List<String> difference = itemsInDifferentStates(transitionName, itemState);
        return difference.isEmpty();
    }

    private List<String> itemsInDifferentStates(String transitionName, String ... itemState) {
        List<String> states = Arrays.asList(itemState);
        List<String> identifiers = this.getIdentifiers() != null ? this.getIdentifiers() : new ArrayList<>();
        List<String> identifiersInState = new ArrayList<>();
        ...
        if (process != null) {
            this.process.keySet().forEach(ident -> {
                ...
                if (zProcess.getTransitionName() != null && ident.endsWith(transitionName) && zProcess.getTransitionName().equals(transitionName)) {
                    if (states.contains(zProcess.getState())) {
                        String rIdent = ident.substring(0, ident.length() - ("_" + transitionName).length());
                        identifiersInState.add(rIdent);
                    }
                }
            });

        }
        Collection<String> subtract = CollectionUtils.subtract(identifiers, identifiersInState);
        return subtract.stream().collect(Collectors.toList());
    }
